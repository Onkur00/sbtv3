...
export const FullscreenChannelPanel: React.FC<FullscreenChannelPanelProps> = ({
  ...
  // If the active channel is not present in the currently selected overlay category,
  // automatically change overlay category to 'all' on panel open so that the playing channel can be highlighted
  useEffect(() => {
    if (isOpen && activeChannelUrl) {
      const indexInCurrentCat = channelsToRender.findIndex(ch => ch.url === activeChannelUrl);
      if (indexInCurrentCat === -1) {
        setFsCategory('all');
      }
    }
  }, [isOpen, activeChannelUrl, channelsToRender]);
  ...
  return (
    <div 
      id="fullscreenPanel" 
      className="absolute inset-0 bg-slate-950/30 backdrop-blur-md p-5 z-50 flex flex-col justify-end gap-3 select-none transition-all duration-300 animate-slide-up"
    >
      {/* Panel Header */}
      <div className="panel-header flex flex-col md:flex-row md:items-center justify-between gap-3 pb-3 border-b border-white/10 mt-auto">
        ...
        <button
          ...
          className="absolute top-4 right-4 text-white bg-slate-900/60 hover:bg-slate-800/80 h-8 w-8 text-sm flex items-center justify-center rounded-full cursor-pointer focus-visible:outline-2 focus-visible:outline-white/40 focus-visible:outline-offset-2 outline-hidden transition-colors border border-white/5 backdrop-blur-md"
          tabIndex={0}
          title="Close guide"
        >
          ✕
      ...
      <div 
        ...
        className="grid grid-cols-[repeat(auto-fill,minmax(76px,1fr))] sm:grid-cols-[repeat(auto-fill,minmax(96px,1fr))] gap-2 max-h-[50vh] overflow-y-auto py-2 pr-1 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-white/20"
      >
            ...
            {channelsToRender.slice(0, fsVisibleCount).map((ch) => {
              ...
              return (
                <div
                  ...
                  className={`fs-channel-item fs-channel-card p-1.5 rounded-2xl text-center cursor-pointer transition-all border outline-hidden flex flex-col justify-center items-center hover:-translate-y-0.5 hover:bg-white/10 hover:scale-[1.02] focus-visible:outline-2 focus-visible:outline-white/40 focus-visible:scale-[1.02] ${
                    isActive 
                      ? 'border-white bg-white/10 shadow-xl font-bold scale-[1.02]' 
                ...
                >
                  <img 
                    ...
                    className={`w-full h-full rounded-xl mx-auto object-cover block transition-transform duration-300 ${
                      isActive ? 'shadow-lg shadow-white/20 border border-white' : 'shadow-md border border-white/5 hover:border-white/20'
                    }`}
                    loading="lazy"
                    onError={(e) => {
