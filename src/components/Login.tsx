...
export const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
  const [username, setUsername] = useState(() => {
    return localStorage.getItem('tv_remembered_username') || '';
  });
  const [password, setPassword] = useState(() => {
    return localStorage.getItem('tv_remembered_password') || '';
  });
  ...
  const handleLogin = (e?: React.FormEvent) => {
    ...
    // Lookup user in credentials folder
    ...
    if (user) {
      ...
      localStorage.setItem('tv_logged_in_user', JSON.stringify(user));

      // Handle Remember Me credentials saving
      if (rememberMe) {
        localStorage.setItem('tv_remembered_username', parsedUsername);
        localStorage.setItem('tv_remembered_password', parsedPassword);
      ...
      <div className="w-full max-w-md relative z-10 transition-all duration-300">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-black tracking-tight text-white font-sans">
            LIVE <span className="text-white">TV</span> PLAYER
          </h1>
        </div>

        {/* Login Card */}
        <div className="bg-slate-900/40 border-2 border-slate-800/40 rounded-2xl p-6.5 shadow-2xl backdrop-blur-md">
