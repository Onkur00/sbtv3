...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <div className="sticky top-0 z-20 bg-transparent pb-1">
      {/* Search Bar Section */}
      <div className="mx-5 mt-1.5 mb-1 bg-slate-950/25 backdrop-blur-md px-4 py-1.5 rounded-full border border-white/10 flex items-center justify-between gap-3 shadow-lg">
        ...
        {/* User Pill Info / Sign out */}
        {currentUser && (
          <div className="flex items-center gap-2">
            <button
              id="logoutBtn"
              onClick={handleLogoutClick}
              className="bg-red-950/70 hover:bg-red-900 border border-red-500/30 text-rose-400 hover:text-white text-[11px] font-bold py-1.5 px-3.5 rounded-full transition-all focus-visible:outline-3 focus-visible:outline-yellow-400 cursor-pointer uppercase tracking-wider flex items-center gap-1 shadow-md shadow-red-950/20"
              title="Logout Account"
            >
              <span>🚪</span> <span className="font-mono font-black">LOGOUT</span>
            </button>
          </div>
        )}
      </div>
