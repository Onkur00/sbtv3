...
export default function App() {
  ...
  const moveFocus = useCallback((dir: 'left' | 'right' | 'up' | 'down') => {
    ...
    const getTier = (el: HTMLElement) => {
      ...
      if (type === 'topBar' || type === 'playerControls' || type === 'qualityOpt') {
        return 'TOP_CONTROLS';
      }
      if (type === 'category') {
        return 'CATEGORIES';
      }
      if (type.startsWith('card-row-') || type === 'card' || type === 'strip') {
        return 'CHANNELS';
      }
      return 'OTHER';
    };
    ...
    if (dir === 'left' || dir === 'right') {
      ...
      // TOP_CONTROLS <-> CATEGORIES <-> CHANNELS
      ...
      if (!isCurrentlyFullscreen && (activeTier === 'TOP_CONTROLS' || activeTier === 'CATEGORIES' || activeTier === 'CHANNELS')) {
        if (dir === 'up') {
          if (activeTier === 'CHANNELS') {
            ...
            // Must go to top panel / player controls
            candidates = focusable.filter(el => getTier(el) === 'TOP_CONTROLS');
          } else if (activeTier === 'TOP_CONTROLS') {
            candidates = focusable.filter(el => getTier(el) === 'TOP_CONTROLS');
          }
        } else if (dir === 'down') {
          if (activeTier === 'TOP_CONTROLS') {
            // Must go to categories bar first
            const categories = focusable.filter(el => getTier(el) === 'CATEGORIES');
            ...
            // Must go down to the channel cards
            candidates = focusable.filter(el => getTier(el) === 'CHANNELS');
  ...
  return (
    <div className="h-screen text-[#f1f5f9] select-none flex flex-col relative overflow-hidden bg-transparent">
      ...
      <div className="flex-1 overflow-y-auto pb-24 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-yellow-400 bg-transparent">
        <ChannelContainer
          activeCategory={activeCategory}
          filteredChannels={filteredChannels}
